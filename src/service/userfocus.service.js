...
class UserfocusService {
    async getshowfollowList(offset, size, id) {
      const statement = `
      select count(1) over()'count', 
 u.name 'name' ,  u.avatar_url 'avatar',u.sign 'sign', 1 'isfollowing'
from user_focus uf
left JOIN  user u
...
limit ? ,?
    ...
    async getshowfansList(offset, size, id) {
      const statement = `
      select count(1) over()'count',
 u.name 'name' ,  u.avatar_url 'avatar',u.sign 'sign', if(
...
limit ? , ?
    ...
    async getshowrecommendsList(offset, size, id) {
      const statement = `
      select distinct B.focus_user_id from user_focus A
         ...
         AND C.focus_user_id is NULL limit ?,?
        `
      const res = await pool.execute(statement, [id,id,id, offset, size])
      return res[0]
    }
